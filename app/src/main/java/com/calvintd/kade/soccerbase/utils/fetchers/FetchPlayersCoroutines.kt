    ...
    suspend fun getFetchedPlayers(data: PlayerResponse?): List<Player> {
        return withContext(Dispatchers.Main) {
            ...
            for (i in playerResponseItems.indices) {
                ...
                val player = Player(
                    playerId, playerNationality, playerName, playerTeam, playerBirthDate, playerBirthLocation, playerSignedDate,
                    playerWage, playerDescription, playerPosition, playerThumbnail, playerFanart
                )
